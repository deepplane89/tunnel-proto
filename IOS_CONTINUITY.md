...
- **PROD build** — generated by `./scripts/build.sh` (no flag) or `npm run build`. Strips dev tools. Writes to **two paths**:
  - `dist/game.js` — consumed by Vercel web deploy via `index.html`'s script tag
  - `www/dist/game.js` — mirror for Capacitor's webDir (consumed by `npx cap copy ios` → iOS bundle). Mirror is automatic as of build.sh patch May 17 2026; pre-patch, `www/dist/game.js` was silently stale.
...
The committed `dist/game.js` is always the prod build. `www/dist/game.js` is gitignored (or should be — see section 11).
...
**Important:** `npx cap copy ios` reads from `www/`, not from `dist/`. The JS bundle reaches `www/dist/game.js` via `scripts/build.sh`'s auto-mirror (PROD only). Non-JS assets (CSS, index.html if it's been edited) still need a manual `cp` into `www/`.
...
npm run build                                                       # writes dist/game.js AND www/dist/game.js (PROD)
...
- `www/dist/game.js` should be added to `.gitignore` if it isn't already. It's a build artifact; only `dist/game.js` (the canonical PROD bundle) should be committed. Verify with `git check-ignore www/dist/game.js` next time the repo is touched.
