...
### Branches
- **One git branch: `dev`.** Web (Vercel) and iOS (Capacitor) both consume `dev`. There is no `main` / `production` branch.

### Two BUILDS of the same source (this is the part agents misread)
The word "build" in this project refers to BUILD OUTPUTS of `src/*.js`, not git branches:

- **DEV build** — generated by `./scripts/build.sh --dev`. Includes tuner panels, perf recorder, dev-only stubs. Robert uses this for local testing on his phone via `python3 -m http.server 8080`. DO NOT commit a dev build to the repo — `scripts/verify.sh` refuses to.
- **PROD build** — generated by `./scripts/build.sh` (no flag) or `npm run build`. Strips dev tools, then mirrors **all root-level Capacitor assets** into `www/`:
  - `dist/game.js` → `www/dist/game.js`
  - `index.html` → `www/index.html`
  - `style.capacitor.css` → `www/style.capacitor.css`
  - `style.css` → `www/style.css`
  - `manifest.json` → `www/manifest.json`
  - `privacy.html` → `www/privacy.html`
  - (Source list in `scripts/build.sh` `WWW_MIRROR_FILES` array. Adding a new shared root-level file? Add it there too — see section 2.)

  Vercel reads from root directly. Capacitor reads from `www/`. The mirror is what bridges them.

DEV builds write only to `dist/game.js` (no www/ mirror — the `:8080` server reads `dist/` directly, and DEV bundles must never enter the App Store-bound iOS bundle).

The committed `dist/game.js` is always the prod build. `www/` is gitignored (regenerated by every PROD build).

### Common agent mistake
"The user is testing locally on dev" means they're running the **DEV build** on the local `:8080` server — NOT that they're on a different git branch. Capacitor != production-only; Capacitor consumes whatever `dist/game.js` is sitting in the bundle, which should always be the PROD build.
...
├── www/                                      # Capacitor webDir. `npx cap copy ios`
│   ├── dist/game.js                          # mirrors www/ → ios/App/App/public/.
│   ├── style.capacitor.css                   # PROD `dist/game.js` is mirrored here
│   ├── index.html                            # automatically by scripts/build.sh.
│   └── ...                                   # Other files (CSS, html) need manual cp.
...
**Important:** `npx cap copy ios` reads from `www/`, not from repo root. The contents of `www/` come from `scripts/build.sh`'s auto-mirror, run by `npm run build` (PROD only). When adding a new root-level asset the app references at runtime (a new HTML page, a new CSS file, a new top-level image), append it to `WWW_MIRROR_FILES` in `scripts/build.sh` — otherwise it will silently not reach the iOS bundle and the app will load an old copy (or no copy at all).

`capacitor.config.json` is the one exception: it has its own bundle path (`ios/App/App/capacitor.config.json`) outside `www/`, so it's `cp`'d separately in the build chain.
...
npm run build                                                       # builds dist/game.js + mirrors all shared assets to www/
cp capacitor.config.json ios/App/App/capacitor.config.json         # config has its own bundle path, not under www/
npx cap copy ios                                                    # mirror www/ → ios/App/App/public/
cd ios/App
...
This is what Robert pastes after every agent change. Shorter than the pre-May-17 version because `npm run build` now does all the asset-mirroring that previously had to be done by hand.

### Verifying the bundle is actually fresh (added May 17 2026 after stale-bundle bug)

After `xcodebuild` finishes but before `devicectl install`, confirm the `App.app` actually contains the latest JS:

```bash
APP=$(find ~/Library/Developer/Xcode/DerivedData/App-*/Build/Products/Debug-iphoneos -name "App.app" -not -path "*Index.noindex*" | head -1)
head -c 50 "$APP/public/dist/game.js"    # should print: /* JH_BUILD: prod */ window.__JH_DEV__=false;
stat -f "%Sm  %z bytes" "$APP/public/dist/game.js"
```

If the first line says `JH_BUILD: dev` or the mtime is from a previous day, **stop and investigate** — the auto-mirror from `build.sh` or `npx cap copy ios` failed. Do not install a stale bundle.
...
- **May 19, 2026 (audio resume audit — NO fixes needed)** — Did a full audit of the Capacitor/TestFlight audio resume system after Robert asked about swipe-away glitches. **Verdict: audio system is structurally sound. Do not refactor without a real bug report.** Tested cold-boot (music plays before first tap on TestFlight build 2 — confirmed by Robert), tested swipe-to-app-switcher during gameplay (music resumes cleanly), tested pause-menu return (no double-play, no stutter). One theoretical gap exists: `engine-start` element gets paused on hide (line 111 of `72-main-late-mid.js`) but is NOT in `allTracks()` so it's not in the snapshot/restore set. Means if user swipes away during the ~3-sec engine roar at run start, engine sound goes silent on return. Narrow window, low impact, NOT indicative of a larger pattern (engine-start is the only audio element outside `allTracks()`; SFX use BufferSourceNodes which are fire-and-forget). Decision: leave as-is. Future agent: do NOT 'fix' this without a user-reported repro. One reverted commit (`3124216`, audio-ready gate on `02-ios-lifecycle.js`) reverted in `0e90ddd` after Robert's cold-boot test disproved the underlying theory — lesson: test the actual behavior on device before shipping fixes for theoretical races.
