...
- **PROD build** — generated by `./scripts/build.sh` (no flag) or `npm run build`. Strips dev tools. This is what ships to:
  - **Vercel** (web production)
  - **Capacitor iOS bundle** (App Store + on-device testing via xcodebuild)

Both outputs go to `dist/game.js` — same path, different contents. Toggle by re-running build.sh. The committed `dist/game.js` is always the prod build.
