  ...
  switch (def.id) {
    ...
    case 'laser': {
      ...
      if (tier <= 3) {
        ...
        if (_lsfx && !isSfxMuted()) {
          _lsfx.loop = false;
          _lsfx.volume = 0.02 * (typeof sfxMult === 'function' ? sfxMult() : 1); // 2026-05-19: 0.2→0.12→0.06→0.04→0.02 — dial down further per user feedback
          ...
          state._laserSfxIv = setInterval(() => {
            ...
            try { _lsfx.volume = 0.02 * (typeof sfxMult === 'function' ? sfxMult() : 1); _lsfx.currentTime = 0; _lsfx.play().catch(()=>{}); } catch(_) {}
          }, _retriggerMs);
