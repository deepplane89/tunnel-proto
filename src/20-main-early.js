...
const TRACK_VOL = { title: 0.4, bg: 0.45, l3: 0.45, l4: 0.45, lake: 0.28, keepgoing: 0.7, radio: 0.8 };
...
function _makeMatForSkinSlot(skinIdx, slotName) {
  ...
  if (def.holo) {
    ...
    mat.userData._jhSession = true; // session-owned: safe to dispose on swap
...
function applyTitleSkin(skinIndex) {
  ...
  if (_wantKey !== (_curAltKey || null)) {
    ...
    if (_newSrc) {
      ...
      // touched.
      try {
        // Walk OLD ship, collect every material in use. Per Three.js docs:
        //   - Materials: must be disposed explicitly (frees shader programs).
        //   - Geometries: SHARED with cached source (clone(true) shares geo) —
        //     do NOT dispose, would break cache.
        //   - Textures: SHARED via Material.clone() (clone copies refs only) —
        //     do NOT dispose, would break cache.
        //   - Holos: also unregister from _holoMaterials registry so the tick
        //     loop stops touching them.
        // The OLD ship's materials at this point are session-owned: paint loop
        // overwrote cache-shared mats with _makeMatForSkinSlot results before
        // we got here. Cones (?:Shader) are also session-owned (built in
        // _thrInit) but get torn down by resetThrusterAnchors below — still
        // dispose them here defensively in case order ever changes.
        const _mats = [];
        const _seen = new Set();
        const _collect = (m) => {
          if (m && !_seen.has(m)) { _seen.add(m); _mats.push(m); }
        };
...
let _cruisePitchFreq = 1.1;       // (unused)
let _cruiseYawAmp  = 0.012;       // rot.y noise amplitude (rad ~0.7°) — small, heading mostly steady
let _cruiseBankAmp = 0.036;       // rot.z noise amplitude (rad ~2.1°) — dominant axis, ship wobbles in roll
let _cruiseXAmp    = 0.012;       // pos.x noise amplitude (units)
...
function _buildCanyonSlabGeo(seed, thickOverride, snapOverride, wOverride) {
  ...
  const H     = T.slabH;
  // wOverride lets entrance slabs extend along Z (corridor direction) so they
  // physically span the regulars-behind range and occlude them at reveal time.
  // Default slabW (20) is correct for regular slabs.
  const W     = (wOverride !== undefined) ? wOverride : T.slabW;
...
function _createCanyonWalls() {
  ...
  function makeSlab(side, seed, zPos, idx, thickOverride) {
    ...
    // Entrance Z-length: extend forward so the slab physically covers the Z
    // range where regulars sit behind it. Front regular at entranceEnd-SPACING
    // (-170), regular pool extends to ~-240 at init. ENTRANCE_W=200 gives the
    // entrance a Z footprint from its pivot Z forward by 200u, fully blocking
    // the regulars-behind range from camera view. Pivot.z is the back edge
    // (z0=0..W in geo) so visually entrance covers [pivot.z, pivot.z+W] in Z.
    const ENTRANCE_W = 200;
    const _entW      = isEntrance ? ENTRANCE_W : undefined;
    const geo        = _buildCanyonSlabGeo(seed, thickOverride, _entSnap, _entW);
